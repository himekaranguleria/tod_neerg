...
// If the user is already logged out send them to returnLogoutUrl
        if (Yii::app()->user->isGuest)
            $this->redirect(Yii::app()->array('/user/login'));
...
// $this->redirect(array('login'));
    ...
    public function actionResetpassword($id) {
        ...
        $user->sendEmail($email, $sub, $view);
        Yii::app()->user->setFlash('reset', '<i class="fa fa-thumbs-o-up"></i>Reset password email is sent to user email.');
        $user->save();
...
//            echo '</per>';
            ...
            if (!$user) {
                ...
                if ($model1->role_id == null)
                    $model1->role_id = User::ROLE_USER;
                if ($model1->save()) {
...
//                    }
    ...
    public function actionUpdate($id = null) {
        ...
        if (isset($_POST ['User'])) {
            $model->setAttributes($_POST ['User']);
//            if (@$_POST['options'])
//                $model->makers = implode(',', $_POST['options']);
//            else
//                $model->makers = '';
